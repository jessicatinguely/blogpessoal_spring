...
public class TemaController {
	...
	@Autowired
	private TemaRepository temaRepository; // Spring Boot faz a injeção de dependência, ou seja, instancia o objeto automaticamente
	...
	public ResponseEntity<Tema> post(@Valid @RequestBody Tema tema) {
		...
		return ResponseEntity.status(HttpStatus.CREATED).body(temaRepository.save(tema)); // salva o tema e retorna o status 201 (CREATED)
	...
	public ResponseEntity<Tema> put(@Valid @RequestBody Tema tema) {
		...
		return temaRepository.findById(tema.getId()) // verifica se o id existe no banco de dados
				.map(resposta -> ResponseEntity.status(HttpStatus.OK)
						.body(temaRepository.save(tema))) // se encontrar o id, atualiza o tema e retorna o status OK
